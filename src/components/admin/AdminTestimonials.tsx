...
export const AdminTestimonials = () => {
  ...
  const [form, setForm] = useState({ name: "", role: "", text: "", rating: 5, sort_order: 0 });
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from("testimonials" as any).update(payload as any).eq("id", editing);
      if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    } else {
      const { error } = await supabase.from("testimonials" as any).insert([payload] as any);
      if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    }

    setEditing(null);
    setIsAdding(false);
    setForm({ name: "", role: "", text: "", rating: 5, sort_order: 0 });
    fetchData();
  ...
  const startEdit = (t: Testimonial) => {
    setEditing(t.id);
    setIsAdding(false);
    setForm({ name: t.name, role: t.role, text: t.text, rating: t.rating, sort_order: t.sort_order });
  };

  const startAdd = () => {
    setIsAdding(true);
    setEditing(null);
    setForm({ name: "", role: "", text: "", rating: 5, sort_order: items.length + 1 });
  };
