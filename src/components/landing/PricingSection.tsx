...
interface DbPrice {
  ...
  promotional_price: number | null; is_promotion_active: boolean; promo_expires_at: string | null;
}

interface DbFeature {
  id: string; certificate_id: string; text: string; icon: string; sort_order: number;
}
...
function Countdown({ expiresAt }: { expiresAt: string }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => { const t = getTimeRemaining(expiresAt); setTime(t); if (!t) clearInterval(interval); }, 1000);
  ...
  return (
    <div className="mt-2 text-center">
      ...
      <div className="flex items-center justify-center gap-1 font-mono text-sm font-bold text-primary">
        <span className="bg-primary/10 rounded px-1.5 py-0.5">{String(time.h).padStart(2, "0")}h</span>:<span className="bg-primary/10 rounded px-1.5 py-0.5">{String(time.m).padStart(2, "0")}m</span>:<span className="bg-primary/10 rounded px-1.5 py-0.5">{String(time.s).padStart(2, "0")}s</span>
      </div>
    </div>
  );
}
...
interface PricingSectionProps {
  city: string; detected?: boolean; onTrackPurchase?: (value: number, productName: string) => void;
}
...
function readList(settings: Record<string, string>, prefix: string, defaults: string[], jsonKey?: string): string[] {
  ...
  if (jsonKey && settings[jsonKey]) {
    try { return JSON.parse(settings[jsonKey]); } catch {}
  }
  return defaults;
}
...
export const PricingSection = ({ city, detected = false, onTrackPurchase }: PricingSectionProps) => {
  ...
  const cpfIdeal = settings.pricing_cpf_ideal || "Pessoa física, profissionais liberais e quem precisa acessar sistemas oficiais, assinar documentos e operar com mais praticidade no ambiente digital.";
  const cnpjIdeal = settings.pricing_cnpj_ideal || "Empresas que precisam emitir notas, cumprir obrigações fiscais e acessar sistemas com segurança e agilidade.";
  ...
  const { data: prices } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("certificate_prices").select("id, name, price, promotional_price, is_promotion_active, promo_expires_at"); return data as unknown as DbPrice[] | null; },
    refetchInterval: 60000,
  });

  const { data: features } = useQuery({
    queryKey: ["certificate_features"],
    queryFn: async () => { const { data } = await supabase.from("certificate_features" as any).select("*").order("sort_order"); return data as unknown as DbFeature[] | null; },
    refetchInterval: 60000,
  });
  ...
  return (
    <section className="bg-background py-16 md:py-24">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <h2 className="mb-12 text-center text-xl font-bold text-foreground sm:text-2xl md:text-3xl lg:text-4xl">{sectionTitle}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto items-stretch">
          {products.map((product) => {
            ...
            const isBestseller = bestsellerActive && ((bestsellerProduct === "cpf" && isCpf) || (bestsellerProduct === "cnpj" && !isCpf));

            return (
              <div key={product.id} className={`relative rounded-2xl border bg-card p-6 md:p-8 flex flex-col ${isBestseller ? "border-primary shadow-lg" : "border-border"}`}>
                {isBestseller && (
                ...
                <div className="mt-4 text-center">
                  {promoActive ? (
                    <div>
                      <span className="text-lg text-muted-foreground line-through">R$ {product.price.toFixed(2).replace(".", ",")}</span>
                      <p className="text-4xl font-black text-primary">R$ {product.promotional_price!.toFixed(2).replace(".", ",")}</p>
                    </div>
                  ) : (
                    <p className="text-4xl font-black text-primary">R$ {product.price.toFixed(2).replace(".", ",")}</p>
                  )}
                </div>
                ...
                <div className="pt-6 mt-auto">
                  <WhatsAppButton buttonId={`cta_pricing_${product.name.toLowerCase().replace(/\s+/g, "")}`}
                    message={isCpf ? getMessage("cta_ecpf", city) : getMessage("cta_ecnpj", city)}
                    className="w-full text-base py-4 bg-whatsapp text-whatsapp-foreground hover:bg-whatsapp/90"
                    onBeforeNavigate={() => { const price = promoActive ? product.promotional_price! : product.price; onTrackPurchase?.(price, product.name); }}>
                    {ctaText}
                  </WhatsAppButton>
                  <p className="mt-2 text-center text-xs text-muted-foreground">{microText}</p>
                </div>
              </div>
            );
          })}
